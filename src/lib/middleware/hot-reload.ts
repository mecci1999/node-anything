...
export default function HotReloadMiddleware(star) {
  ...
  function watchProjectFiles() {
    if (!star.started || (!process.mainModule && !require.main)) return;
    ...
    if (extraFiles != null) {
      Object.entries(extraFiles).forEach(([fName, restartType]) => {
        ...
        if (restartType == 'star') watchItem.starRestart = true;
    ...
    const reloadServices = _.debounce(() => {
      const needToReloadDedup = _.uniqWith([...needToReload], (a, b) => {
        const ac = typeof a == 'string' ? a : (a as any).__filename;
        const bc = typeof b == 'string' ? b : (b as any).__filename;
        return ac == bc;
      });
      ...
      needToReloadDedup.forEach((svc) => {
        if (typeof svc == 'string')
          if (fs.existsSync(svc)) return star.loadService(svc);
          else return;
   ...
   * @param {Number} level
   */
  function processModule(mod: any, service: any = null, level: number = 0, parents: any = null) {
    ...
    if ((service || parents) && fName.indexOf('node_modules') !== -1)
      if (hotReloadModules.find((modulePath) => fName.indexOf(modulePath) !== -1) == null) return;
    ...
    if (service) {
      ...
      const watchItem = getWatchItem(fName);
      if (!watchItem.services.includes(service.fullName)) watchItem.services.push(service.fullName);

      watchItem.others = _.uniq([].concat(watchItem.others, parents || []));
    } else if (isMoleculerConfig(fName)) {
      const watchItem = getWatchItem(fName);
      watchItem.starRestart = true;
    } else {
  ...
  return {
    ...
    // After star started
    started(star) {
      if (star.options.hotReload == null) {
        return;
      } else if (typeof star.options.hotReload === 'object') {
        if (Array.isArray(star.options.hotReload.modules)) {
          hotReloadModules = star.options.hotReload.modules.map((moduleName) => `/node_modules/${moduleName}/`);
        }
        if (star.options.hotReload.extraFiles) {
          /**
           * **Example:**
           * ```js
           * hotReload: {
           * 	extraFiles: {
           * 		"./configuration.json": "star", // reload the star
           ...
           * ```
           */
          extraFiles = star.options.hotReload.extraFiles;
        }
      } else if (star.options.hotReload !== true) {
        return;
      }
    ...
    serviceStarted() {
      // Re-watch new services if star has already started and a new service started.
      if (star.started) {
